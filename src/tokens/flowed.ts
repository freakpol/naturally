...
const resolverInputTransformedWithRegEx = /param ([._a-zA-Z][.\-_a-zA-Z0-9]+) transformed with ((?<object>\{(.+)\});|(?<array>\[(.*)\]);|(?<string>"(.+)");)/is;
...
const resolverInputFixedObjectRegEx = /param ([._a-zA-Z][.\-_a-zA-Z0-9]+) with value ((?<object>\{(.+)\});|(?<array>\[(.*)\]);)/is;
...
const regexTest = (regex: RegExp, startString: string, text: string, startOffset: number, lineEndings: string | string[]) => {
  ...
  const subtext = text.substr(startOffset, stringLength);
...
const resolverInputTransformedWithPattern = (text: string, startOffset: number) => {
  const execResult = regexTest(resolverInputTransformedWithRegEx, 'param ', text, startOffset, ['];', '};', '";']);
  if (execResult !== null) {
    let from: any;
...
const resolverInputFixedNullOrBoolPattern = (text: string, startOffset: number) => {
  const execResult = regexTest(resolverInputFixedNullOrBoolRegEx, 'param ', text, startOffset, '\n');
  if (execResult !== null) {
    // @ts-ignore
    execResult.payload = {
      fixed: {
...
const resolverInputFixedObjectPattern = (text: string, startOffset: number) => {
  const execResult = regexTest(resolverInputFixedObjectRegEx, 'param ', text, startOffset, ['};', '];']);
  if (execResult !== null) {
    let from: any;
